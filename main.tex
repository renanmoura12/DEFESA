...
\end{itemize}
 ...
 O \textit{SMOTE} é uma técnica de pré-processamento desenvolvida por \textcite{Chawla2002} para lidar com o problema de desbalanceamento de classes em conjuntos de dados, situação comum em aplicações médicas onde eventos de interesse (como doenças raras ou complicações) são significativamente menos frequentes que casos normais. O algoritmo funciona gerando amostras sintéticas da classe minoritária através de interpolação linear entre instâncias existentes e seus k-vizinhos mais próximos, em vez de simplesmente replicar amostras existentes. Esta abordagem permite expandir o espaço de características da classe minoritária, melhorando a capacidade do modelo de aprender padrões relevantes e reduzindo o viés em favor da classe majoritária. Na área da saúde, o SMOTE tem sido amplamente utilizado em problemas como detecção de fraudes em seguros, diagnóstico de doenças raras e predição de complicações clínicas \cite{Chawla2002, Elreedy2019}.

\subsection{DATA AUGMENTATION GAUSSIANA}

A \textit{Data Augmentation Gaussiana} é uma técnica que adiciona ruído gaussiano controlado aos dados de treinamento para melhorar a robustez e a capacidade de generalização dos modelos de aprendizado de máquina. Esta abordagem é particularmente relevante em aplicações médicas, onde os dados frequentemente apresentam variações naturais decorrentes de diferentes equipamentos de aquisição, protocolos ou condições ambientais. Ao treinar modelos com dados ligeiramente perturbados, o algoritmo aprende a focar nas características essenciais e diagnósticas, ignorando variações superficiais irrelevantes. Estudos demonstram que o Gaussian Noise Up-Sampling (GNUS) pode apresentar desempenho comparável ou superior ao SMOTE em datasets clínicos de pequeno porte, sendo especialmente eficaz para melhorar a precisão de algoritmos de segmentação tumoral e classificação de imagens médicas \cite{Beinecke2021, Chlap2021}.

\subsection{SHAP (SHapley Additive exPlanations)}

O \textit{SHAP} é um framework de explicabilidade desenvolvido por Lundberg e Lee (2017) que utiliza conceitos da teoria dos jogos cooperativos para interpretar as predições de modelos de machine learning. O método atribui a cada variável um valor de importância (valor de Shapley) que quantifica sua contribuição para uma predição específica, permitindo compreender como cada característica influencia o resultado do modelo. Na área da saúde, a explicabilidade é fundamental para a adoção clínica de sistemas de IA, pois profissionais de saúde necessitam compreender o raciocínio por trás das predições para validar decisões e estabelecer confiança. O SHAP tem sido aplicado em diversas áreas médicas, incluindo predição de risco cardiovascular, diagnóstico de câncer, detecção de Alzheimer e estratificação de pacientes, contribuindo para aumentar a transparência e confiabilidade dos modelos preditivos em ambientes clínicos \cite{Lundberg2017, Islam2024}.
...
Dado que uma mesma paciente pode ter realizado múltiplas consultas durante a gestação, foi 
necessário identificar os episódios gestacionais únicos. Para isso, implementou-se um 
algoritmo que percorre as consultas de cada paciente em ordem cronológica, utilizando o 
identificador único (\textit{PacienteId}) como chave de agrupamento. Consultas separadas 
por intervalo superior a 270 dias (aproximadamente 9 meses) foram consideradas episódios 
gestacionais distintos, recebendo um sufixo alfabético sequencial para rastreabilidade 
(ex: \textit{1124}, \textit{1124A}). Esse procedimento possibilitou diferenciar múltiplas 
consultas de uma mesma gestação de novas gestações da mesma paciente, resultando em 151 
episódios gestacionais avaliados — dos quais 127 sem o desfecho de pré-eclâmpsia e 24 que tiveram o desfecho.
