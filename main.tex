...
O modelo final implementado foi o LightGBM (LGBMClassifier) de gradiente boosting otimizado para classificação binária, por demonstrar melhor desempenho com o tamanho de amostra disponível. Foi treinado com 18 variáveis clínicas: idade, IMC, diabetes, hipertensão, peso no primeiro trimestre, origem racial, histórico familiar de diabetes, tipo de diabetes, média do índice de pulsatilidade (IP), perdas gestacionais, peso, idade gestacional, idade gestacional corrigida, peso fetal, percentil da artéria uterina, percentil da artéria umbilical, percentil de peso fetal e circunferência abdominal. Configurado com n\_estimators=500, learning\_rate=0,1, num\_leaves=31, max\_depth=6 e regularização (reg\_alpha=0,1 e reg\_lambda=0,1), o modelo alcançou AUC-ROC de 0,9711 e acurácia de 0,9318, com precisão, recall e F1 de 0,93 no conjunto de teste independente (22 casos com e 22 sem pré-eclâmpsia).

\begin{quadro}[H]
\centering

{\bfseries Quadro 1: Dados de entrada para predição}
...
{\bfseries Quadro 2: Dados de entrada para predição (continuação)}
...
Após a aprovação do Comitê de Ética, os dados clínicos e ultrassonográficos foram exportados dos sistemas da clínica e unificados em uma única planilha, na qual as diferentes tabelas foram integradas por meio do campo \textit{PacienteId}. A preparação dos dados foi aplicada diretamente na pipeline da ML, no qual realizamos a conversão explícita dos tipos de variáveis, datas, números inteiros e reais. A padronização de categorias textuais como cor/raça, histórico familiar de diabetes, e o cálculo da idade materna a partir da data de nascimento e de uma data de referência consistente para cada paciente. Valores ausentes foram tratados com regras específicas: uso da moda para variáveis categóricas, substituição por zero para variáveis que representam ausência de condição clínica e utilização de valores de referência quando não foi possível inferir a informação a partir dos registros.
