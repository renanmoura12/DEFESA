...
\vspace{4cm}
{\textbf{DESENVOLVIMENTO DE UM MODELO DE APRENDIZADO DE MÁQUINA PARA PREDIÇÃO DE PRÉ-ECLÂMPSIA SEM USO DE BIOMARCADORES LABORATORIAIS: UMA ABORDAGEM BASEADA EM DADOS CLÍNICOS E LAUDOS ULTRASSONOGRÁFICOS DE ROTINA}}
...
\textbf{Orientador}: Prof. Dr. Wellington Candeia de Araújo.
\vfill
...
]{imagens/fichaCatalografica.pdf}
...
\parbox{10cm}{
\singlespacing{\begin{hyphenrules}{nohyphenation}
{\large Dedico a Tamiris, minha esposa e companheira de vida, por seu apoio incondicional. Agradeço por sua paciência, compreensão e por estar ao meu lado em cada passo desta jornada.}
\end{hyphenrules}
}}
\end{flushright}
\thispagestyle{empty}
...
Aos meus familiares, Adriana, Romero, Luiza e Fabiana por todo o apoio e incentivo, sem os quais não teria conseguido chegar até aqui.
\\

Ao Professor Dr. Frederico Moreira Bublitz, por sua orientação e paciência ao longo deste trabalho.
\\

Ao meu colega do mestrado, Leonardo, por todo o apoio e parceria durante a realização deste trabalho.
\\

À minha colega de mestrado, Bartira, que se tornou amiga para a vida, por toda a ajuda dentro e fora da academia.
\\

À Clínica Embrion, que contribuiu com os dados e apoio técnico para a realização deste estudo, sem os quais este trabalho não teria sido possível.
...
\noindent Pre-eclampsia (PE) is one of the leading causes of maternal and perinatal morbidity and mortality. The condition is characterized by the onset of hypertension after 20 weeks of gestation and may trigger a range of serious complications. In severe cases, it can lead to fetal growth restriction and preterm birth, significantly increasing the risk of neonatal complications, including death. Therefore, early detection and appropriate management are essential to minimize adverse outcomes for both mother and newborn. The Fetal Medicine Foundation (FMF) developed the Pre-eclampsia Risk Calculator, which combines maternal factors, mean arterial pressure, uterine artery pulsatility indices, and serum biomarkers such as placental growth factor (PLGF) through a Bayesian algorithm. This tool has been widely recognized and used since the early 2010s. Despite its advances, this method was validated using data that may not reflect the Brazilian population, introducing diagnostic imprecision that can compromise the quality of prenatal care and perinatal outcomes. In recent years, machine learning techniques have emerged as promising tools in medical applications, enabling the identification of patterns in large volumes of clinical data that may indicate disease onset before overt symptoms appear. Addressing this limitation, a machine learning model was developed and trained on real clinical data from our local population. The model was internally validated and demonstrated high-precision results. It was made available through a REST API to allow integration with health information systems, aiming to optimize pre-eclampsia risk prediction for pregnant women.
...
Com os avanços em ultrassonografia e marcadores bioquímicos, \textcite{Nicolaides2011_PrenatalCarePyramid} propôs a inversão desta pirâmide, deslocando o foco principal para o primeiro trimestre (11-13 semanas) da gravidez. Esse novo paradigma permite a detecção precoce de riscos para várias complicações gestacionais, incluindo malformações, PE, restrição de crescimento fetal, diabetes gestacional e parto prematuro, por meio da combinação de características maternas, marcadores biofísicos, como pressão arterial média e marcadores bioquímicos, como \textit{PAPP-A e PlGF}. A mudança de paradigma representou uma transição de um modelo reativo para um modelo preventivo, onde a identificação precoce de gestantes de alto risco possibilita a implementação de profilaxia, ao invés de simplesmente aguardar a complicação ocorrer para então tratá-la \cite{SONEK2016305}.

A \textit{Fetal Medicine Foundation} (FMF) foi uma percursora neste novo método e desenvolveu uma calculadora de risco para PE que combina dados da história materna, pressão arterial média e índices de pulsatilidade das artérias uterinas, além de outros biomarcadores, para estimar o risco de desenvolver PE durante a gestação. A calculadora utiliza o teorema de Bayes para combinar o risco prévio derivado de fatores maternos com dados adicionais coletados durante a gestação, permitindo uma avaliação mais precisa do risco de PE \cite{Riishede2023}. A utilização dessa ferramenta pode ajudar na identificação precoce de gestantes de risco, permitindo intervenções preventivas e melhorando os resultados maternos e fetais. No entanto, a calculadora da FMF foi desenvolvida e validada principalmente com dados de populações europeias, o que pode limitar sua precisão quando aplicada a outras populações, como a brasileira, devido a diferenças genéticas, ambientais e socioeconômicas \cite{Rezende2024}.
...
O estudo tem como benefício principal o desenvolvimento de um modelo preditivo para predição de PE utilizando somente dados clínicos, contribuindo para a identificação precoce de gestantes em risco e auxiliando na melhoria dos desfechos materno-fetais, como também trará benefícios acadêmicos e científicos, com publicações em revistas nacionais e internacionais.
...
Para contextualizar e fundamentar o trabalho desenvolvido, abordaremos o conhecimento teórico sobre os temas que fazem parte do estudo.
...
Os biomarcadores clínicos são ferramentas que ampliam a acurácia diagnóstica e fundamentam a medicina de precisão, como observado no câncer de pulmão, no qual assinaturas genéticas orientam terapias direcionadas e auxiliam no diagnóstico e monitoramento de doenças, permitindo que os médicos identifiquem e tratem condições de saúde de forma mais precisa e personalizada. \cite{Xavier2022}.
...
O fator de crescimento placentário (\textit{Placental Growth Factor} -- PlGF) e a proteína plasmática-A associada à gravidez (\textit{Pregnancy-Associated Plasma Protein-A} -- PAPP-A) são marcadores bioquímicos dosados em amostras de soro materno entre 11 e 13 semanas de gestação. O PlGF atua na angiogênese placentária e seus níveis reduzidos precedem as manifestações clínicas da PE, enquanto o PAPP-A está envolvido na regulação do crescimento fetal e na remodelação vascular. Apesar de sua relevância clínica, a dosagem desses biomarcadores demanda imunoensaios automatizados em plataformas específicas e reagentes de alto custo, o que restringe sua aplicabilidade em contextos de baixa disponibilidade tecnológica e financeira, como grande parte da rede de atenção primária no Brasil \cite{Chaemsaithong2022}.

\begin{figure}[H]
    \centering
    \caption{Kit de reagentes Elecsys PAPP-A para plataforma Cobas e analyzers.}
    \includegraphics[width=\textwidth, height=0.6\textheight, keepaspectratio]{imagens/PAPPa.png}
    \par\centering\small Fonte: Roche Diagnostics, 2026.
\end{figure}
...
\end{itemize}
 ...
 O \textit{SMOTE} é uma técnica de pré-processamento desenvolvida por \textcite{Chawla2002} para lidar com o problema de desbalanceamento de classes em conjuntos de dados, situação comum em aplicações médicas onde eventos de interesse (como doenças raras ou complicações) são significativamente menos frequentes que casos normais. O algoritmo funciona gerando amostras sintéticas da classe minoritária através de interpolação linear entre instâncias existentes e seus k-vizinhos mais próximos, em vez de simplesmente replicar amostras existentes. Esta abordagem permite expandir o espaço de características da classe minoritária, melhorando a capacidade do modelo de aprender padrões relevantes e reduzindo o viés em favor da classe majoritária. Na área da saúde, esse método tem sido amplamente utilizado em problemas como detecção de fraudes em seguros, diagnóstico de doenças raras e predição de complicações clínicas \cite{Elreedy2019}.

\subsection{DATA AUGMENTATION GAUSSIANA}

A \textit{Data Augmentation Gaussiana} é uma técnica que adiciona ruído gaussiano controlado aos dados de treinamento para melhorar a robustez e a capacidade de generalização dos modelos de aprendizado de máquina. Esta abordagem é particularmente relevante em aplicações médicas, onde os dados frequentemente apresentam variações naturais decorrentes de diferentes equipamentos de aquisição, protocolos ou condições ambientais. Ao treinar modelos com dados ligeiramente perturbados, o algoritmo aprende a focar nas características essenciais e diagnósticas, ignorando variações superficiais irrelevantes \cite{Beinecke2021, Chlap2021}.

\subsection{SHAP (SHapley Additive exPlanations)}

O \textit{SHAP} é um framework de explicabilidade desenvolvido por \textcite{Lundberg2017}, que utiliza conceitos da teoria dos jogos cooperativos para interpretar as predições de modelos de machine learning. O método atribui a cada variável um valor de importância, o valor de \textit{Shapley}, que quantifica sua contribuição para uma predição específica, permitindo compreender como cada característica influencia o resultado do modelo. Na área da saúde, a explicabilidade é fundamental para a adoção clínica de sistemas de IA, pois profissionais de saúde necessitam compreender o raciocínio por trás das predições para validar decisões e estabelecer confiança. O SHAP tem sido aplicado em diversas áreas médicas, incluindo predição de risco cardiovascular, diagnóstico de câncer, detecção de Alzheimer e estratificação de pacientes, contribuindo para aumentar a transparência e confiabilidade dos modelos preditivos em ambientes clínicos \cite{Islam2024}.

\section{API REST}

Uma \textit{API REST (Representational State Transfer)} é um padrão arquitetônico para comunicação entre sistemas que utiliza os princípios do protocolo \textit{HTTP} para facilitar a comunicação entre clientes e servidores. Utiliza um conceito de \textit{endpoints}, que são os pontos de comunicação do sistema, no qual se pode enviar ou receber dados por meio de uma URL. Segundo \cite{gowda2024}, sua eficiência reside na adoção de práticas como: (1) design orientado a recursos, em que entidades (como usuários ou produtos) são acessadas via \textit{endpoints} (ex: /api/v1/usuarios); (2) operações sem estado, garantindo que cada requisição contenha informações completas para processamento, eliminando dependências de sessões anteriores; e (3) uso explícito de métodos \textit{HTTP (GET, POST, PUT, DELETE)}para mapear operações \textit{CRUD (Create, Read, Update, Delete)}. Essa abordagem traz benefícios significativos: escalabilidade, pois a ausência de conexões persistentes permite distribuir carga entre servidores; segurança robusta, com implementação de HTTPS e protocolos como \textit{JWT} para autenticação; e flexibilidade, viabilizando integração facilitada entre sistemas. Além disso, técnicas como \textit{caching} e paginação otimizam desempenho, reduzindo latência em cenários de alto tráfego\cite{gowda2024}. A adoção dessas práticas não apenas padroniza o desenvolvimento, mas também reduz riscos de vulnerabilidades e custos de manutenção, posicionando \textit{APIs REST} como pilares essenciais para aplicações \textit{web} modernas.
...
FMF (regressão) \cite{Rezende2024}
...
Araújo \textit{et al.} (LightGBM) \cite{Araujo2024}
...
Bulez \textit{et al.} (DL / Extra Trees) \cite{Bulez2024}
...
Li \textit{et al.} (Stacking) \cite{Li2024}
& Dados maternos; PAM; UTA-PI; PLGF; PAPP-A
& 0,884
& --
& --
& Validação em centro único; uso de \textit{SHAP} para explicabilidade. \\

Schmidt \textit{et al.} (ML ensemble) \cite{Schmidt2022}
...
Edvinsson \textit{et al.} (XGBoost) \cite{Edvinsson2024}
& Variáveis clínicas maternas
& 0,90 (CV) / 0,85 (teste)
& --
& --
& Coorte piloto extremamente reduzida (n=81); uso de Optuna e \textit{SHAP}; foco em necessidade de UTI. \\
...
A metodologia do desenvolvimento do modelo PPML seguiu o processo de pipeline de aprendizado de máquina, baseado no modelo proposto por \textcite{Kreuzberger2023MLOps}, adaptado à nossa proposta de aplicação clínica.
\begin{figure}[H]
    \centering
...
A validação do modelo ocorreu de forma interna, utilizando nossos próprios dados que não participaram do treinamento nem do balanceamento das classes, sendo o conjunto de teste composto por 42 dados, dos quais 20 apresentaram PE e 22 não tiveram o desfecho. Para cada dado o modelo produziu probabilidades de ocorrência, onde a partir delas foram estimadas as curvas \textit{Receiver Operating Characteristic} (ROC) e \textit{precision-recall} e a partir dessas curvas, foram calculadas métricas de desempenho, incluindo área sob a curva (AUC), acurácia, sensibilidade, especificidade, precisão, \textit{F1-score} e matriz de confusão, com ênfase na análise da sensibilidade em diferentes pontos de corte.

\section{Disponibilização do modelo}
...
Este estudo foi submetido e aprovado pelo Comitê de Ética em Pesquisa (CEP), em conformidade com as diretrizes estabelecidas pela Resolução nº 466/2012 do Conselho Nacional de Saúde (CNS), que regulamenta pesquisas envolvendo seres humanos. O parecer consubstanciado de aprovação, de número 7.974.575, encontra-se disponível no \autoref{anx:parecer_cep}. Os dados clínicos utilizados neste estudo foram coletados de forma retrospectiva, garantindo o anonimato dos pacientes e a confidencialidade das informações, sendo utilizados exclusivamente para fins científicos.
...
Os exames selecionados para o estudo foram escolhidos por apresentarem grande variedade de informações ultrassonográficas e dados maternos, sendo provenientes de atendimentos clínicos rotineiros. Os tipos de exames incluídos foram: obstétrico, obstétrico com Doppler e translucência nucal, por serem geralmente o ciclo normal de exames da gravidez. Como critério de inclusão, definiu-se que pacientes com um ou mais desses exames deveriam ter seus dados exportados juntamente com o questionário de desfecho da gravidez, parte do protocolo de acompanhamento de gestantes que realizaram pré-natal completo na clínica. Essa estratégia permitiu avaliar o acompanhamento longitudinal por gestante durante todo o período gestacional.
...
Dado que uma mesma paciente pode ter realizado múltiplas consultas ao longo da gestação, foi necessário consolidar essas consultas em registros únicos por episódio gestacional. Para isso, implementou-se um algoritmo que percorre as consultas de cada paciente em ordem cronológica, utilizando o \textit{PacienteId} como chave de agrupamento, e consolida todas as consultas de um mesmo período gestacional em um único registro. Quando identificado um intervalo superior a 270 dias (aproximadamente 9 meses) entre consultas consecutivas, caracterizou-se o início de uma nova gestação, gerando um novo identificador com sufixo alfabético sequencial para rastreabilidade (ex: \textit{1124}, \textit{1124A}). Esse procedimento resultou em 151 episódios gestacionais avaliados.
...
Valores ausentes foram tratados segundo regras específica, onde variáveis que representam ausência de condição clínica foram preenchidas com zero, e quando não foi possível inferir a informação a partir dos registros disponíveis,foram utilizados valores de referência clínica ou médias populacionais.
...
Testes sucessivos de treinamento e validação interna foram conduzidos para identificar as variáveis que efetivamente impactavam a capacidade preditiva do modelo. Para quantificar a contribuição de cada variável, foi utilizado o método SHAP (\textit{SHapley Additive exPlanations}), que mensura o impacto individual de cada preditor sobre as predições do modelo. Variáveis que apresentaram importância nula ou próxima de zero foram sistematicamente excluídas. Essa baixa importância decorreu principalmente da escassez de ocorrências na base de dados, inviabilizando a geração de dados sintéticos representativos e tornando sua inclusão inadequada do ponto de vista metodológico.
...
Adicionalmente, foram aplicados limites fisiologicamente plausíveis às variáveis contínuas, idade materna (15–50 anos), peso materno (35–150 kg), IMC (15–50 kg/m²) e peso fetal (0–5.000 g), com o objetivo de reduzir a influência de outliers e mitigar o impacto de erros de digitação sem descartar registros completos.

A pipeline completa de preparação dos dados foi implementada em Python, utilizando as bibliotecas \textit{pandas} e \textit{scikit-learn}, integrando todas as etapas descritas acima.

\section{Resultados}

O modelo alcançou AUC-ROC de 0,9886 e acurácia de 0,9524, com precisão de 0,9500, sensibilidade de 0,9500 e F1-score de 0,9500. A avaliação incluiu curva ROC, matriz de confusão, curva de aprendizado e análise de importância das variáveis pelo método SHAP, onde o ranking destacou como principais preditores: hipertensão, idade, peso, tipo de diabetes, IMC, peso fetal, idade gestacional, origem racial, percentil da artéria uterina e percentil da artéria umbilical, o que corresponde ao que é descrito como fatores de risco para a condição na literatura \cite{Maayeh_2020}. O código completo do treinamento está disponível no \autoref{anx:algoritmo_treinamento}.

\begin{figure}[H]
    \centering
    \caption{Métricas de avaliação do modelo LightGBM: curva ROC, importância das variáveis e curva de aprendizado.}
...
Diante do tamanho reduzido da amostra disponível, a avaliação de overfitting foi tratada como etapa prioritária na validação do modelo. A comparação sistemática entre as métricas obtidas nos conjuntos de treino e teste revelou diferenças inferiores a 0,05 em todas as métricas avaliadas: AUC-ROC (0,9942 vs 0,9886), acurácia (0,9851 vs 0,9524), precisão (0,9796 vs 0,9500), recall (0,9897 vs 0,9500) e F1-score (0,9846 vs 0,9500), indicando que o modelo não apresenta sinais significativos de memorização dos dados de treino e generaliza de forma consistente para dados não vistos. Esse resultado reforça a adequação das estratégias adotadas, como o agrupamento por paciente via \texttt{GroupShuffleSplit}, que garante que todos os registros de uma mesma paciente permaneçam inteiramente em apenas um dos conjuntos, evitando vazamento de dados. A estratificação por desfecho e o uso controlado de \textit{data augmentation} contribuíram conjuntamente para mitigar os riscos inerentes ao treinamento com amostras reduzidas.


\begin{figure}[H]
    \centering
    \caption{Análise de sobreajuste: comparação entre métricas de treino e teste do modelo LightGBM.}
...
\begin{figure}[H]
    \centering
    \caption{Documentação interativa OpenAPI/Swagger da API PPML.}
    \includegraphics[width=\textwidth, height=0.6\textheight, keepaspectratio]{imagens/swagger.png}
    \par\centering\small Fonte: Adaptado de Swagger, 2026.
\end{figure}
...
O presente estudo desenvolveu e validou o PPML \textit{Predictive Preeclampsia Machine Learning}, um modelo preditivo de PE baseado em aprendizado de máquina que utiliza exclusivamente variáveis clínicas e ultrassonográficas coletadas rotineiramente durante o acompanhamento pré-natal. O modelo final, implementado com o algoritmo \textit{LightGBM}, demonstrou desempenho superior ao de modelos previamente descritos na literatura, com elevada capacidade de detecção precoce e baixas taxas de falsos positivos e negativos.

A estratégia metodológica adotada, que incluiu seleção criteriosa de variáveis por meio de análise \textit{SHAP}, técnicas de balanceamento de dados \textit{(SMOTE)} e \textit{data augmentation gaussiana}, mostrou-se fundamental para atingir o desempenho observado. As cinco variáveis de maior importância identificadas foram : hipertensão, idade, peso, tipo de diabetes e IMC, todas passíveis de obtenção em contextos assistenciais de rotina, sem necessidade de biomarcadores laboratoriais específicos e condizentes com a literatura.

A disponibilização do modelo por meio de uma \textit{API REST}, desenvolvida em \textit{Python} com \textit{FastAPI} e hospedada em ambiente \textit{cloud}, viabiliza a integração da ferramenta em sistemas de informação em saúde existentes, permitindo sua aplicação em tempo quase real durante o atendimento pré-natal. Essa arquitetura possibilita que profissionais de saúde possam obter estimativas de risco individualizadas, contribuindo para estratificação mais precisa das gestantes e direcionamento de recursos e intervenções preventivas.

Reconhece-se, entretanto, algumas limitações importantes do estudo. O tamanho amostral relativamente reduzido, embora tenha permitido resultados promissores, pode limitar a generalização do modelo para populações com características demográficas, clínicas ou étnicas distintas. A origem dos dados em um único centro de atendimento também pode introduzir vieses relacionados ao perfil específico da população atendida e aos protocolos locais de assistência pré-natal. Além disso, em decorrência da escassez de registros disponíveis, o treinamento do modelo dependeu de técnicas de geração de dados sintéticos, o que implica que parte das amostras utilizadas durante o aprendizado podem não corresponder a observações clínicas reais. Contudo, a arquitetura do PPML foi projetada para permitir o retreinamento com novos dados reais à medida que estes se tornem disponíveis, de modo que a incorporação progressiva de registros clínicos reais tende a reduzir a dependência de dados sintéticos e a fortalecer a capacidade de generalização do modelo. A validação foi conduzida de forma interna, sem validação externa em coortes independentes, o que representa uma etapa essencial para confirmação da robustez e aplicabilidade clínica do modelo.
...
Ao final, concluímos que o PPML representa uma contribuição relevante para a predição de PE no contexto brasileiro, oferecendo uma alternativa tecnicamente viável, de baixo custo operacional e potencialmente implementável em larga escala em qualquer sistema que possua os dados necessários para a predição, como o Sistema Único de Saúde (SUS). Apesar das limitações apontadas, os resultados obtidos indicam que a abordagem proposta pode auxiliar na identificação precoce de gestantes em risco, contribuindo para a redução de desfechos adversos materno-fetais associados à condição.
...
\includepdf[
  pages=-,
  pagecommand={}
]{imagens/lightgbm_modelo_final.pdf}

\chapter{Parecer Consubstanciado do CEP}
\label{anx:parecer_cep}
\thispagestyle{myheadings}
